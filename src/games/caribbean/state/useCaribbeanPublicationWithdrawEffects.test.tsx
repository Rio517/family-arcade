...
describe('useCaribbean withdrawBattle publication effects', () => {
  it.each(['governor', 'tavern', 'market', 'shipyard', 'shares', 'log'] as const)(
    ...
    async (activity) => {
      ...
      await act(() => invoke(hook.result.current, 'withdrawBattle'));

      expect(hook.result.current.journal?.events.at(-1)).toMatchObject({ type: 'battle-withdrawn' });
